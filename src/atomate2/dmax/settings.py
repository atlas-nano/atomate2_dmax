...
class DmaxSettings(Atomate2Settings):
    ...
    LAMMPS_RUN_COMMAND: str = Field(
        default="lmp_serial",
        description="Command to run LAMMPS simulations"
    )
    
    # Data storage settings
    STORE_VOLUMETRIC_DATA: bool = Field(
        default=True,
        description="Whether to store volumetric data from simulations"
    )
    
    STORE_TRAJECTORY: bool = Field(
        default=True,
        description="Whether to store MD trajectories"
    )
    
    # LAMMPS computational settings
    LAMMPS_INCAR_UPDATES: dict = Field(
        default_factory=dict,
        description="Updates to apply to LAMMPS input files"
    )
    
    LAMMPS_KSPACING: float = Field(
        default=0.5,
        description="K-point spacing for LAMMPS calculations"
    )
    ...
    DEFAULT_FF_NAME: str = Field(
        default="oplsaa",
        description="Default force field name (oplsaa, gaff2, etc.)"
    )
    
    # GPU settings
    USE_GPU: bool = Field(
        default=False,
        description="Whether to use GPU acceleration for LAMMPS"
    )
    
    GPU_COUNT: int = Field(
        default=1,
        description="Number of GPUs to use for LAMMPS calculations"
    )
