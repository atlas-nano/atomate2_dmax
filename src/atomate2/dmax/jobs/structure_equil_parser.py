...
class StructureEquilParserMaker(Maker):
    ...
    def make(
        ...
        if labels is None:
            raise RuntimeError(
                f"Could not find thermo_style in input script {input_script}"
            )
        ...
        if not data:
            raise RuntimeError(
                f"No thermo data lines found in log {log_path} for labels {labels}"
            )
        ...
        try:
            ...
            EinsteinMSD = msd_mod.EinsteinMSD
        ...
        # load universe
        u = mda.Universe(
            data_file, dumps, topology_format="DATA", format="LAMMPSDUMP", dt=0.001
        )
        # MSD
