...
class GlassTransitionPlotMaker(Maker):
    ...
    def make(
        self, parser_docs: list[DmaxDmaParserDocument], temperatures: list[float]
    ) -> Response:
        # extract metrics
        ...
        tans = np.array(
            [d.tan_delta if d.tan_delta is not None else np.nan for d in parser_docs],
            dtype=float,
        )
        ...
        with open(data_csv, "w", newline="") as f_csv:
            ...
            for T, sm, lm, td, em, pr in zip(
                temps, storages, losses, tans, elastics, poissons
            ):
        ...
        ax1.plot(
            temps, storages, color="tab:blue", linestyle="-", label="Storage Modulus"
        )
