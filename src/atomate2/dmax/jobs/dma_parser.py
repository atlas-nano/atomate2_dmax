...
class DmaParserMaker(Maker):
    ...
    def make(
        ...
        with open(input_script) as f:
            for line in f:
                ...
                if parts and parts[0] == 'fix' and 'deform' in parts:
                    ...
                    if len(parts) > idx + 2:
                        axis = parts[idx + 2]
        if amp_pc is None or period is None or axis is None or temperature_val is None or pressure_val is None or dt_fs is None:
        ...
        guess_phi = 0.0
        popt, _ = curve_fit(lambda tt, A, phi: sin_func(tt, A, phi, omega), t, p, p0=[guess_A, guess_phi])
        A_fit, phi_fit = popt
        ...
        if dLy.max() == max_diff:
            axial_strain, lateral1_strain, lateral2_strain = strain_y, strain_x, strain_z
        elif dLx.max() == max_diff:
            axial_strain, lateral1_strain, lateral2_strain = strain_x, strain_y, strain_z
        else:
            axial_strain, lateral1_strain, lateral2_strain = strain_z, strain_x, strain_y
        ...
        ax3.text(0.05, 0.95, f'Elastic energy (slope): {slope_ellipse:.2f} MPa',
