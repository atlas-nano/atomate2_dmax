...
class LammpsNVESet(BaseLammpsSetGenerator):
    ...
    def __init__(self, **kwargs):
        ...
        self.settings.update(
            {k: v for k, v in kwargs.items() if k in _BASE_LAMMPS_SETTINGS.keys()}
        )
        kwargs = {
            k: v for k, v in kwargs.items() if k not in _BASE_LAMMPS_SETTINGS.keys()
        }
        self.settings.update(
            {
...
class LammpsMinimizeSet(BaseLammpsSetGenerator):
    ...
    def __init__(
        self, max_steps: int = 10000, pressure: float = 0, tol: float = 1.0e-6, **kwargs
    ):
